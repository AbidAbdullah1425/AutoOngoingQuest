        ...
        try:
            if self.msg_id:
                ...
                if ss_path and sp_path:
                    ...
                    self.logger.info(f"All steps completed successfully for file {self.output_file}.")
                    try:
                        shutil.rmtree(_hash)
                        os.remove(sp_path)
                        ...
                        self.logger.warning("Failed to clean up temporary files.")
        except BaseException:
            error_msg = str(format_exc())
            self.logger.error(f"Error during further work: {error_msg}")
            await self.reporter.report_error(error_msg, log=True)
